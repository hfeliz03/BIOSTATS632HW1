...
```{r namename, echo=FALSE, clean = FALSE , include=T}
x <- plot_cp_hw1(est, iso_code = 4)
plot(x)
```
...
```{r name4, warning=FALSE, include=T}
a <- plot_cp(dat, est, iso_code = 4)
b <- plot_cp(dat, est, iso_code = 4, CI = NA)
c <- plot_cp(dat, est, iso_code = 404, CI = 80)
plot(a)
plot(b)
plot(c)
```
...
https://github.com/hfeliz03/BIOSTATS632HW1
